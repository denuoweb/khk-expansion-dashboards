...
const TaskCard: React.FC<TaskCardProps> = ({ task, roles, onStatusChange, onEdit, onDelete }) => {
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm hover:shadow-md transition-shadow touch-manipulation ${
      isOverdue ? 'border-red-300 bg-red-50' : 'border-gray-200'
    }`}>
      <div className="p-3 sm:p-4">
        {/* Header */}
        <div className="flex items-start justify-between mb-3">
          <h4 className="font-semibold text-gray-900 flex-1 pr-2 text-sm sm:text-base">{task.title}</h4>
          <div className="relative">
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-6 bg-white border border-gray-200 rounded-lg shadow-lg z-20 min-w-[120px]">
                  <button
                    onClick={() => {
                      onEdit(task);
                      setShowMenu(false);
                    }}
                    className="flex items-center w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 touch-manipulation"
                  >
        ...
        <button
          ...
          className="flex items-center space-x-1 text-xs sm:text-sm text-blue-600 hover:text-blue-700 mb-3 touch-manipulation"
        >
        ...
        {isExpanded && (
          <div className="bg-gray-50 rounded-lg p-3 mb-3">
            <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{task.description}</p>
        ...
        {nextStatus && nextStatusLabel && (
          <button
            ...
            className={`w-full flex items-center justify-center space-x-2 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors touch-manipulation ${
